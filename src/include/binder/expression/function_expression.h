...
public:
    // TODO(Xiyang): I don't think we need to pass expressionType here.
    ScalarFunctionExpression(common::ExpressionType expressionType,
        function::ScalarFunction function,
        std::unique_ptr<function::FunctionBindData> bindData, expression_vector children,
        std::string uniqueName)
        : Expression{expressionType, *bindData->resultType, children, uniqueName}, function{function}, bindData{std::move(bindData)}  {}
    ...
    //        const std::string& uniqueName)
    //        : ScalarFunctionExpression{std::move(functionName), expressionType,
    //        std::move(bindData),
...
//          aggregateFunction{std::move(aggregateFunction)} {}
    ...
    bool isDistinct() const { return aggregateFunction->isFunctionDistinct(); }
