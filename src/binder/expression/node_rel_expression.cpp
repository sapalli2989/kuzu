...
bool NodeOrRelExpression::hasPrimaryKey() const {
    for (auto& e : propertyExprs) {
        if (e->constCast<PropertyExpression>().isPrimaryKey()) {
            return true;
        }
    }
    return false;
...
std::shared_ptr<Expression> NodeOrRelExpression::getPrimaryKey() const {
    KU_ASSERT(hasPrimaryKey());
    for (auto& e : propertyExprs) {
        if (e->constCast<PropertyExpression>().isPrimaryKey()) {
            return e->copy();
        }
