...
static std::unique_ptr<ScalarFunction> bindCastFromStringFunction(const std::string& functionName,
    ...
    case LogicalTypeID::DECIMAL: {
        ...
        case PhysicalTypeID::INT16:
            execFunc = ScalarFunction::UnaryExecNestedTypeFunction<ku_string_t, int16_t, CastToDecimal>;
            break;
        case PhysicalTypeID::INT32:
            execFunc = ScalarFunction::UnaryExecNestedTypeFunction<ku_string_t, int32_t, CastToDecimal>;
            break;
        case PhysicalTypeID::INT64:
            execFunc = ScalarFunction::UnaryExecNestedTypeFunction<ku_string_t, int64_t, CastToDecimal>;
            break;
        case PhysicalTypeID::INT128:
            execFunc = ScalarFunction::UnaryExecNestedTypeFunction<ku_string_t, int128_t, CastToDecimal>;
            break;
        default:
            KU_UNREACHABLE;
        }
...
static std::unique_ptr<ScalarFunction> bindCastFromRdfVariantFunction(
    ...
    return std::make_unique<ScalarFunction>(functionName,
        std::vector<LogicalTypeID>{LogicalTypeID::RDF_VARIANT}, targetType.getLogicalTypeID(), execFunc);
}
...
static std::unique_ptr<ScalarFunction> bindCastToStringFunction(const std::string& functionName,
    ...
    case LogicalTypeID::DECIMAL: {
        ...
        case PhysicalTypeID::INT64:
            ...
            break;
        case PhysicalTypeID::INT128:
            func = ScalarFunction::UnaryExecNestedTypeFunction<int128_t, ku_string_t, CastDecimalTo>;
            break;
        default:
            KU_UNREACHABLE;
        }
...
static std::unique_ptr<ScalarFunction> bindCastBetweenNested(const std::string& functionName,
    ...
    default:
        throw ConversionException{stringFormat("Unsupported casting function from {} to {}.",
            LogicalTypeUtils::toString(sourceType.getLogicalTypeID()), LogicalTypeUtils::toString(targetType.getLogicalTypeID()))};
    }
}
...
template<typename DST_TYPE>
static std::unique_ptr<ScalarFunction> bindCastBetweenDecimalFunction(const std::string& functionName,
    const LogicalType& sourceType) {
    scalar_func_exec_t func;
    switch(sourceType.getPhysicalType()) {
    case PhysicalTypeID::INT16:
    ...
    return std::make_unique<ScalarFunction>(functionName, std::vector<LogicalTypeID>{LogicalTypeID::DECIMAL},
...
function_set CastToStringFunction::getFunctionSet() {
    ...
    for (auto& type : LogicalTypeUtils::getAllValidLogicTypes()) {
        auto function = CastFunction::bindCastFunction(name, LogicalType::makeLogicalTypeUnsafe(type), *LogicalType::STRING());
...
function_set CastToDoubleFunction::getFunctionSet() {
    ...
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::DOUBLE()));
    }
    result.push_back(
...
function_set CastToFloatFunction::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::FLOAT()));
    }
    result.push_back(
...
function_set CastToInt128Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::INT128()));
    }
    result.push_back(
...
function_set CastToSerialFunction::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::SERIAL()));
    }
    result.push_back(
...
function_set CastToInt64Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::INT64()));
    }
    result.push_back(
...
function_set CastToInt32Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::INT32()));
    }
    result.push_back(
...
function_set CastToInt16Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::INT16()));
    }
    result.push_back(
...
function_set CastToInt8Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::INT8()));
    }
    result.push_back(
...
function_set CastToUInt64Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::UINT64()));
    }
    result.push_back(
...
function_set CastToUInt32Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::UINT32()));
    }
    result.push_back(
...
function_set CastToUInt16Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::UINT16()));
    }
    result.push_back(
...
function_set CastToUInt8Function::getFunctionSet() {
    function_set result;
    for (auto typeID : LogicalTypeUtils::getNumericalLogicalTypeIDs()) {
        result.push_back(CastFunction::bindCastFunction(name, LogicalType(typeID), *LogicalType::UINT8()));
    }
    result.push_back(
...
static std::unique_ptr<FunctionBindData> castBindFunc(const binder::expression_vector& arguments,
    ...
    func->execFunc = CastFunction::bindCastFunction(func->name,
        arguments[0]->getDataType(), targetType)
                         ->execFunc;
