...
struct SourceState {
    ...
    explicit SourceState(nodeID_t sourceNodeID) {
...
struct VariableLengthPathLocalState : public AlgoFuncLocalState {
    ...
    void materializeFrontier(nodeID_t sourceNodeID, const Frontier& frontier, uint8_t length,
        FactorizedTable& table) const {
        srcNodeIDVector->setValue<nodeID_t>(0, sourceNodeID);
        for (auto dstNodeID : frontier.getNodeIDs()) {
            dstNodeIDVector->setValue<nodeID_t>(0, dstNodeID);
            auto numPath = frontier.getMultiplicity(dstNodeID);
    ...
    for (auto offset = 0u; offset < graph->getNumNodes(); ++offset) {
        ...
        for (auto currentLevel = 1; currentLevel <= extraData->upperBound; ++currentLevel) {
            ...
            if (currentLevel >= extraData->lowerBound) {
                localState->materializeFrontier(sourceNodeID, sourceState.nextFrontier, currentLevel, output);
            }
