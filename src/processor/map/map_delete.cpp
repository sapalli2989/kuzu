...
ExtraNodeDeleteInfo PlanMapper::getExtraNodeDeleteInfo(table_id_t tableID) {
    ...
    return ExtraNodeDeleteInfo(table, std::move(fwdRelTables), std::move(bwdRelTables));
}

std::unique_ptr<NodeDeleteExecutor> PlanMapper::getNodeDeleteExecutor(const BoundDeleteInfo& boundInfo, const Schema& schema) {
    ...
    if (node.isMultiLabeled()) {
        ...
        for (auto id : node.getTableIDs()) {
            extraInfos.insert({id, getExtraNodeDeleteInfo(id)});
        }
        return std::make_unique<MultiLabelNodeDeleteExecutor>(std::move(info), std::move(extraInfos));
    }
    auto extraInfo = getExtraNodeDeleteInfo(node.getSingleTableID());
...
std::unique_ptr<RelDeleteExecutor> PlanMapper::getRelDeleteExecutor(const BoundDeleteInfo& info, const Schema& schema) {
    auto sm = clientContext->getStorageManager();
