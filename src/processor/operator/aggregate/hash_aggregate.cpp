...
void HashAggregate::executeInternal(ExecutionContext* context) {
    ...
    // 1
    // 2
    auto ft = localState.aggregateHashTable->getFactorizedTable();
    for (auto i = 0u; i < localState.aggregateHashTable->getFactorizedTable()->getNumTuples();
         i++) {
        auto col1 = ft->getData<int64_t>(0, i, 0);
        auto col2 = ft->getData<bool>(0, i, 1);
        auto c = 5;
    }
